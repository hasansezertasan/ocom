"""Tests for the package's runnable entrypoint (``python -m ocom``)."""
...
def test_main_is_callable() -> None:
    ...
    ADR-007) targets ``ocom.__main__:main``, so this pins the
    ...
    """
    main_module = importlib.import_module("ocom.__main__")
...
def test_console_root_reports_missing_launcher_dependency(
    ...
    Given:
        - An environment out of sync with the installed package, so one of the
          ...
          ``uv sync``.
    When:
        - ``__main__`` loads the console root, as bare ``ocom`` does.
    Then:
...
def test_console_root_preserves_unrelated_import_errors(
    ...
    """
    main_module = importlib.import_module("ocom.__main__")
    monkeypatch.setitem(sys.modules, "ocom.cli", None)

    with pytest.raises(ModuleNotFoundError, match=r"ocom\.cli"):
        _ = vars(main_module)["_load_console_root"]()
