...
not consumed by ocom itself. Copy its contents into your
...
3. Bootstrap the initial formula:
   ...
   brew create --set-name ocom <pypi-sdist-url> --tap hasansezertasan/tap
   brew update-python-resources hasansezertasan/tap/ocom
   ```

   These write `Formula/ocom.rb` into brew's own checkout of the
...
5. Push everything so the tap repository actually contains the listener and
   ...
   ```sh
   git add -A
   git commit -m "Add ocom + release listener"
   git push
   ```

## Token setup (in this project's repo)

In **this** project's repository — `hasansezertasan/ocom` — add
