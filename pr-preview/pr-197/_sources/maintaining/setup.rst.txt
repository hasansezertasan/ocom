...
`Contributing Guidelines <https://github.com/hasansezertasan/ocom/blob/main/.github/CONTRIBUTING.md>`_.
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/ocom \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/ocom/branches/main/protection \
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/ocom/actions/permissions/workflow \
...
.. code-block:: sh
   ...
   gh api --silent \
     repos/hasansezertasan/ocom/dependency-graph/compare/main...main
...
.. code-block:: sh
   ...
   gh api -X PUT repos/hasansezertasan/ocom/environments/publish \
     -F 'deployment_branch_policy[protected_branches]=false' \
     -F 'deployment_branch_policy[custom_branch_policies]=true'
   ids="$(gh api --paginate repos/hasansezertasan/ocom/environments/publish/deployment-branch-policies \
     --jq '.branch_policies[].id')"
   for id in $ids; do
     gh api -X DELETE "repos/hasansezertasan/ocom/environments/publish/deployment-branch-policies/$id"
   done
   gh api -X POST repos/hasansezertasan/ocom/environments/publish/deployment-branch-policies \
...
.. code-block:: sh
   ...
   gh variable set ENABLE_PRIVATE_ATTESTATIONS --body true --repo hasansezertasan/ocom
...
.. code-block:: sh
   ...
   policies="$(gh api --paginate repos/hasansezertasan/ocom/environments/publish/deployment-branch-policies \
...
.. code-block:: sh
   ...
   priv="$(gh repo view hasansezertasan/ocom --json isPrivate --jq '.isPrivate' 2>/dev/null)"
   if [ "$priv" = "false" ]; then true
   elif [ "$priv" = "true" ]; then
     gh secret list --repo hasansezertasan/ocom --json name --jq '.[].name' | grep -qx CODECOV_TOKEN
   else false; fi
...
.. code-block:: sh
   ...
   gh secret set GITLEAKS_LICENSE --repo hasansezertasan/ocom
...
.. code-block:: sh
   ...
   owner="$(gh api repos/hasansezertasan/ocom --jq '.owner.type' 2>/dev/null)"
   if [ "$owner" = "User" ]; then true
   elif [ "$owner" = "Organization" ]; then
     gh secret list --repo hasansezertasan/ocom --json name --jq '.[].name' | grep -qx GITLEAKS_LICENSE
   else false; fi
...
.. code-block:: sh
   ...
   gh api repos/hasansezertasan/ocom/pages \
...
.. code-block:: sh
   ...
   [ "$(gh repo view hasansezertasan/ocom --json hasDiscussionsEnabled --jq '.hasDiscussionsEnabled')" = "true" ]
...
.. code-block:: sh
   ...
   gh secret list --repo hasansezertasan/ocom --json name \
...
``https://repology.org/project/ocom/versions`` and verify that
...
``ocom``. If it does, use the name shown by Repology in both
URLs below.
...
.. code-block:: markdown
   ...
   [![Packaging status](https://repology.org/badge/vertical-allrepos/ocom.svg)](https://repology.org/project/ocom/versions)
