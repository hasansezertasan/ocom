...
This is the single runnable entrypoint used by ``python -m ocom``
...
When a ``ocom`` console root exists (``include_console_root`` —
...
_MISSING_ROOT_DEPENDENCY = "Error: The ocom command requires the '{missing}' package, which is not installed. It ships with 'ocom', so this usually means your environment is out of sync -- run `uv sync` (or reinstall the package) and try again."  # noqa: E501
...
def _load_console_root() -> Callable[[], None]:
    ...
    try:
        ...
        from ocom.cli import app  # noqa: PLC0415
    except ModuleNotFoundError as exc:
        missing = exc.name
...
def main() -> None:  # pragma: no cover
    """Run the ocom console root (primary + component subcommands)."""
    _load_console_root()()
